...
(function($){
	...
	Timepicker.prototype = {
		...
		injectTimePicker: function(inst){
			...
			var html = '<div id="ui-timepicker-div">'+
					'<dl>'+
						'<dt id="ui_tpicker_time_label">时间</dt>'+
						'<dd id="ui_tpicker_time"></dd>'+
						'<dt id="ui_tpicker_hour_label"'+ ((inst.defaults.showHour)? '':' style="display:none;"') +'>时</dt>'+
						'<dd id="ui_tpicker_hour"'+ ((inst.defaults.showHour)? '':' style="display:none;"') +'></dd>'+
						'<dt id="ui_tpicker_minute_label"'+ ((inst.defaults.showMinute)? '':' style="display:none;"') +'>分</dt>'+
						'<dd id="ui_tpicker_minute"'+ ((inst.defaults.showMinute)? '':' style="display:none;"') +'></dd>'+
						'<dt id="ui_tpicker_second_label"'+ ((inst.defaults.showSecond)? '':' style="display:none;"') +'>秒</dt>'+
	...
	jQuery.fn.datetimepicker = function(o){
		...
		this.each(function() {
			var $input = $(this);
			var $spanClear = $('<span />').attr('title', '清空时间').addClass(
					'ui-icon').addClass('ui-icon-trash').css('display',
					'inline-block').css('height', '18px').css('cursor',
					'pointer').css('margin-bottom', '-3px');
			$spanClear.click(function() {
				$input.val('');
			});
			$input.after($spanClear);
		});
	};
	...
	$.datepicker._selectDay = function(id, month, year, td) {
		...
		var $curtd = $('td.'+this._currentClass);
		// 修改选中样式
		$curtd.removeClass(this._currentClass);
		$curtd.children('a').removeClass('ui-state-active');
