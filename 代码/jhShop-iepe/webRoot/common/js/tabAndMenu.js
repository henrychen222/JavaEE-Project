				...
				for ( var i in menu.children) {
					...
					if (menu2.children.length > 0) {
						...
						for ( var i in menu2.children) {
							...
							level3.append("<li><a href='javascript:void(0);' "
									+ "onclick='jumpPage(\"" + contextPath
									+ "/" + menu3.uri + "\");'><b><i>"
									+ menu3.title + "</i></b></a></li>");
						}
...
function jumpPage(url,pageName,fn) {
	var tabhtml="<iframe id='iframeContent_"+pageName+"' src='"+url+"' data-name='"+pageName+"'"+
	...
	if(pageExists){
		...
		$('#easyuiTabs').tabs({
			  onBeforeClose: function(title,index){
				 $("#iframeContent_"+title).contents().find("body").empty();
				   return true;
			  }
			});
		...
		jumpPage(url,pageName,fn,1);
	}
}

function jumpPage_(url,pageName,type) {	
	var pageExists=$('#easyuiTabs').tabs('exists',pageName);
	//$("#iframeContent").attr("src", url);
	if(pageExists){
		$('#easyuiTabs').tabs('select',pageName);
