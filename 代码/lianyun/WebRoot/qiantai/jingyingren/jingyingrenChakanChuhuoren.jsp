	...
	<style type="text/css">
      ...
      <script language="javascript">
               ...
               function chuhuorenEdit(id)
           {
            ...
            function apply(id){
           window.location.href="<%=path %>/chuhuoren?type=jingyingrenYunxuApply&id="+id;
           } 
           
           function chakanFeiyong(){
           
           window.location.href="<%=path %>/yunshu?type=yunshuMana";
           }
		   ...
		   function fahuo(id){
		    window.location.href="<%=path %>/chuhuoren?type=chuhuorenFahuo1&id="+id;
		  ...
		  <TR>
		    ...
		    <TD class=b vAlign=top width=591 align=left>
		      ...
		      <TABLE border=0 cellSpacing=0 cellPadding=2 width=590 align=left height=133>
				...
				<tr bgcolor="#E7E7E7">
					<td height="14" colspan="14" background="<%=path %>/img/tbg.gif">出货申请表&nbsp;</td>
				</tr>
				...
				<tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='red';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22">
					...
					<td bgcolor="#FFFFFF" align="center">
						 <div onmouseover = "over('<%=path %>/upload/${chuhuoren.productFujianYuanshiming}')" onmouseout = "out()" style="cursor:hand;">
							查看图片
					   </div>
					</td>
					<c:if test="${chuhuoren.applyStatus=='未申请'}">
					<td bgcolor="#FFFFFF" align="center" style="color:green">
						 ${chuhuoren.applyStatus}
					</td>
					</c:if>
					<c:if test="${chuhuoren.applyStatus=='已申请'}">
					<td bgcolor="#FFFFFF" align="center" style="color:red">
						 ${chuhuoren.applyStatus}
					</td>
					</c:if>
					<c:if test="${chuhuoren.fahuoStatus=='未发货'}">
					<td bgcolor="#FFFFFF" align="center" style="color:green">
						 ${chuhuoren.fahuoStatus}
					</td>
					</c:if>
					<c:if test="${chuhuoren.fahuoStatus=='已发货'}">
					<td bgcolor="#FFFFFF" align="center" style="color:red">
						 ${chuhuoren.fahuoStatus}
					</td>
					</c:if>
					...
					<c:if test="${chuhuoren.applyStatus=='未申请'}">
					<td bgcolor="#FFFFFF" align="center">
						<input type="button" value="允许申请" onclick="apply( ${chuhuoren.id})"/>
						
					</td>
					</c:if>
					<c:if test="${chuhuoren.applyStatus=='已申请'&&chuhuoren.fahuoStatus=='未发货'}">
					<td bgcolor="#FFFFFF" align="center">
						
						<input type="button" value="发货" onclick="fahuo(${chuhuoren.id})"/>
					</td>
					</c:if>
					<c:if test="${chuhuoren.applyStatus=='已申请'&&chuhuoren.fahuoStatus=='已发货'}">
					<td bgcolor="#FFFFFF" align="center">
						
						<input type="button" value="查看费用"  onclick="chakanFeiyong()"/>
					</td>
					</c:if>
				</tr>
				</c:forEach>
